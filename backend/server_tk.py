...
def allow_cors(request):
    ...
    request.setHeader('Access-Control-Max-Age', b'2520')
...
async def get_summoner_overview(request, server, summoner_name):
    ...
    return json.dumps({k: json_data[k] for k in ["name", "summonerLevel", "profileIconId"]})
...
async def get_summoner_account_id(request, server, summoner_name):
    ...
    if is_error(response):
        return get_error_response(response.code)
    json_data = await treq.json_content(response)
    return json.dumps({k: json_data[k] for k in ["accountId", "id"]})
...
async def get_champion_mastery(request, server, summoner_id):
    ...
    if is_error(response):
        return get_error_response(response.code)
    json_data = await treq.json_content(response)
    return json.dumps([{**{k: champ_obj[k] for k in ["championId", "championLevel", "championPoints"]}, 
...
async def get_full_timeline(request, server, account_id):
    ...
    matches = await treq.json_content(matches_response)
    event_type = get_request_param(request, "event")
...
async def get_match_details(request, server, match_id):
    ...
    if is_error(response):
        return get_error_response(response.code)
    json_data = await treq.json_content(response)
    return json.dumps(match_details.get_relevant_details(json_data))
